...
export default {
    ...
    submitForm() {
        ...
        if(val === 1) {
          if(result) {
            ...
            servicioCurso.actualizarId(this.id, this.form).then(response => {
  ...
  watch: {
    id(){
      this.$vs.loading()
      servicioCurso.obtenerId(this.id).then(response => {
            console.log('responseEditarIdi', response)
            this.selecInstructor = {id: response.data.instructorId, label: response.data.instructorNombre}

            this.form.nombreCurso = response.data.nombreCurso
