<template>
    <vx-card title="Editar Mision" id="parentx-demo-5">
        ...
        <vs-row>
          <vs-col  vs-type="flex" vs-justify="center" vs-align="center" vs-lg="3" vs-sm="4" vs-xs="12" >
            Direccion
            <v-select class="selectTa" v-model="selecDireccion" :options="optionsDireccion" />
          </vs-col>
        </vs-row>
    ...
    consularN(){
      ...
      this.$vs.loading()
        consultaEmpleadoServicio.consultarEmpleado(this.form.numeroEmpleado).then(response => {
              ...
              var label2
              (response.data.data[0].data.sexo === 1) ? label2 = 'Policia': label2 = 'Administrativo'
              this.selectTipoE = {id: response.data.data[0].data.tipo_empleado, label: label2}
              console.log('iddd', response.data.data[0].data.sexoId === 1)
