<template>
    <vs-popup class="holamundo" title="Editar" :active.sync="verL">
      <form>
        <vs-input size="large" v-validate="'required'" label="Medio" placeholder="Medio" name="Medio" v-model="form.sexo" class="mt-5 w-full" />
...
export default {
    ...
    submitForm() {
      this.$validator.validateAll().then(result => {
        ...
        if(result) {
          ...
          medio.actualizarId(this.id, this.form).then(response => {
  ...
  watch: {
    id(){
      this.$vs.loading()
      medio.obtenerId(this.id).then(response => {
            console.log('response', response.data.sexo)
