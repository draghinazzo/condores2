<template>
    <vs-popup class="Institucion" title="Formulario" :active.sync="verL">
      <form>
        <vs-input size="large" v-validate="'required'" label="Institucion" placeholder="Institucion" name="Institucion" v-model="form.nombre" class="mt-5 w-full" />
        <span class="text-danger text-sm" v-show="errors.has('Institucion')">{{ errors.first('Institucion') }}</span>
...
<script>
import servicio from '@/services/catalogos/institucion'
