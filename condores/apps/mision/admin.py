from django.contrib import admin
from apps.mision.models import *
# Register your models here.

admin.site.register(catMedio)
admin.site.register(catSolicitante)
admin.site.register(catServicio)
admin.site.register(catCargo)
admin.site.register(persona)
admin.site.register(catInstitucion)
admin.site.register(licencia_persona)
admin.site.register(domicilio_persona)
admin.site.register(instructores)
admin.site.register(descripcion_emergencia)
admin.site.register(solicitud_emergencia)
"""
admin.site.register(solicitante)
admin.site.register(Helicopteros)
admin.site.register(direccionMision)
admin.site.register(mision)
admin.site.register(bitacora)
admin.site.register(tripulacion)
admin.site.register(mantenimiento)
"""


